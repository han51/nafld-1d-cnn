...
# 2) Run this code to obtain error_rate_vector (i.e., error rate for each epoch)
# 3) Compare the values of error_rate_vector obtained from the N sets of hyper parameters.
...
checkpoint_path = '../models_temp/classifier/'
...
with tf.name_scope("cross_ent"):
  # mean cross entropy
  cross_ent_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = fc2, labels = y),
                              name = 'cross_ent_loss')  
  tf.add_to_collection('losses', cross_ent_loss)
...
with tf.name_scope("accuracy"):
  prob = tf.nn.softmax(fc2, dim=-1, name='prob')  
  predicted = tf.argmax(fc2, 1)   
  is_corroct = tf.equal(predicted, tf.argmax(y, 1))
  accuracy = tf.reduce_mean(tf.cast(is_corroct, tf.float32))
  tf.add_to_collection('fc2', fc2)
...
error_rate_matrix = np.zeros([max_num_epochs, num_folds])  

for fold in range(0, num_folds):
  training_file_list = '../data/file_lists_with_labels_classifier/training_t' + str(fold+1) + '.txt'
  val_file_list = '../data/file_lists_with_labels_classifier/training_v' + str(fold+1) + '.txt'  
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(max_num_epochs):
      ...
      for imgi in range(val_batches_per_epoch):
        ...
        y_label_images[imgi] = batch_val_y[:,1].mean()            
        y_raw_matrix[imgi,:] = sess.run(prob, feed_dict={x: batch_val_x}).T[1]
      val_generator.reset_pointer()
      ...
      for idx in val_patient_image_idx:
        ...
        pti = pti+1
      error_rate_matrix[epoch, fold] = np.sum(np.abs((y_raw_pt>0.5) - y_label_pt))/y_raw_pt.size
    
    error_rate_vector = error_rate_matrix.mean(axis=1)
